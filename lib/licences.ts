...
export async function listAthleteLicences(filters: Filters, scope: AccessScope, deps: Dependencies = defaults) {
  ...
  const licences = data.licences.map((row) => view(row, maps)).filter((item) => allowed(scope, item)).filter((item) => !filters.seasonId || item.seasonId === filters.seasonId).filter((item) => !filters.teamId || item.equipeId === filters.teamId).filter((item) => !filters.statusId || item.statusId === filters.statusId).filter((item) => !filters.athleteId || item.athleteId === filters.athleteId).filter((item) => !search || `${item.athleteNom} ${item.athleteId} ${item.numero}`.toLocaleLowerCase("fr").includes(search)).sort((a, b) => b.saison.localeCompare(a.saison, "fr", { numeric: true }) || b.dateDelivrance.localeCompare(a.dateDelivrance) || a.athleteNom.localeCompare(b.athleteNom, "fr"));
