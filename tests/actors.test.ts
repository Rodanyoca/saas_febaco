...
test("aligne les autres acteurs sur les colonnes réelles",()=>{const result=validateActorInput("autres",{nom_complet:"A",id_sexe:"SEX001",statut:"ACTIF",id_type_autre_acteur:"TAU001",entite:"FEBACO",observations:"Mandat",lieu_de_naissance:"Kinshasa"});assert.equal(result.values.entite,"FEBACO");assert.equal(result.values.observations,"Mandat");assert.equal(Object.prototype.hasOwnProperty.call(result.values,"lieu_de_naissance"),false)})
test("lit la colonne physique Observations des autres acteurs",()=>{const actor=normalizeActor("autres",{id_autre_acteur:"AUT1",nom_complet:"Test",Observations:"Observation réelle"});assert.equal(actor.observations,"Observation réelle")})
