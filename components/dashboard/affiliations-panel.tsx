...
const labels: Record<AffiliationKind, string> = { athlete: "athlète", coach: "coach", medecin: "médecin", officiel: "officiel" }
...
export function AffiliationsPanel({ kind, actorId }: { kind: AffiliationKind; actorId: string }) {
  ...
  const headers = kind === "officiel" ? ["Entité", "Type", "Fonction", "Date de début", "Date de fin", "Statut"] : kind === "coach" ? ["Équipe", "Club", "Fonction", "Date de début", "Date de fin", "Statut"] : ["Équipe", "Club", "Date de début", "Date de fin", "Statut"]
  const cells = (item: Item) => kind === "officiel" ? [item.entite || item.id_entite, refs.entityTypes.find((x) => x.id === item.id_type_entite)?.label || item.id_type_entite, item.fonction || item.id_fonction, formatDisplayDate(item.date_debut), item.date_fin ? formatDisplayDate(item.date_fin) : "En cours", item.statut] : kind === "coach" ? [item.equipe, item.club, item.fonction || item.id_fonction, formatDisplayDate(item.date_debut), item.date_fin ? formatDisplayDate(item.date_fin) : "En cours", item.statut] : [item.equipe, item.club, formatDisplayDate(item.date_debut), item.date_fin ? formatDisplayDate(item.date_fin) : "En cours", item.statut]
  ...
  return <Card><CardHeader className="flex-row items-center justify-between gap-3"><CardTitle>Affiliations</CardTitle><div className="flex gap-2"><Button variant="outline" size="icon" onClick={() => void load()} disabled={loading} aria-label="Actualiser les affiliations" title="Actualiser"><RefreshCw className={loading ? "animate-spin" : ""} /></Button>{federal ? <Button onClick={() => start()}><Plus />Ajouter une affiliation</Button> : null}</div></CardHeader><CardContent>
    ...
    {["athlete", "coach", "medecin"].includes(kind) ? <div className="space-y-2"><Label>Équipe</Label><SearchSelect value={values.id_equipe} onValueChange={(value) => set("id_equipe", value)} options={refs.teams} placeholder="Rechercher une équipe" disabled={saving || viewing} />{errors.id_equipe ? <p className="text-sm text-destructive">{errors.id_equipe}</p> : null}</div> : null}{kind === "coach" ? field("id_fonction", "Fonction", "text", refs.coachFunctions) : null}
    ...
    {field("date_debut", "Date de début", "date")}{field("date_fin", "Date de fin (facultative)", "date")}{field("id_statut_affiliation", "Statut", "text", refs.statuses)}{field("observation", "Observation", "textarea")}{errors._form ? <p role="alert" className="text-sm text-destructive">{errors._form}</p> : null}
