...
export default function ActorLicensesPage() {
  ...
  const actions = (x: ActorLicenseView) => <DropdownMenu><DropdownMenuTrigger asChild><Button size="icon-sm" variant="ghost" aria-label={`Actions pour ${x.actorName}`} title="Actions"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent side="top" align="end" className="w-44"><DropdownMenuItem onSelect={() => openEditor("view", x)}><Eye />Consulter</DropdownMenuItem>{federal && x.effectiveStatus !== "ANNULEE" ? <><DropdownMenuItem onSelect={() => openEditor("renew", x)}><RefreshCw />Renouveler</DropdownMenuItem><DropdownMenuItem onSelect={() => openEditor("edit", x)}><Pencil />Modifier</DropdownMenuItem></> : null}<DropdownMenuItem onSelect={() => void navigator.clipboard.writeText(x.id).then(() => toast({ title: "Identifiant copié" }))}><Copy />Copier l’identifiant</DropdownMenuItem>{federal ? <DropdownMenuSeparator /> : null}{federal && ["VALIDE", "A_VENIR"].includes(x.effectiveStatus) ? <DropdownMenuItem onSelect={() => void transition(x, "suspend")}><PauseCircle />Suspendre</DropdownMenuItem> : null}{federal && x.effectiveStatus === "SUSPENDUE" && (x.remainingDays ?? -1) >= 0 ? <DropdownMenuItem onSelect={() => void transition(x, "reactivate")}><RotateCcw />Réactiver</DropdownMenuItem> : null}{federal && ["VALIDE", "A_VENIR", "SUSPENDUE"].includes(x.effectiveStatus) ? <DropdownMenuItem onSelect={() => void transition(x, "close")}><Ban />Clôturer</DropdownMenuItem> : null}{federal && x.effectiveStatus !== "ANNULEE" ? <DropdownMenuItem variant="destructive" onSelect={() => void transition(x, "cancel")}><XCircle />Annuler</DropdownMenuItem> : null}</DropdownMenuContent></DropdownMenu>
