...
export async function GET(req: Request) {
  try {
    ...
    const filterMedecinId = normalizeId(searchParams.get("medecinId"))
    if (!filterMedecinId) {
      return NextResponse.json({ affiliations: [], error: "id_medecin requis." }, { status: 400 })
    }

    const rows = await readSheetRows({
      block: "affiliations",
      sheet: "MEDECINS_AFFILIATIONS",
      range: "A:ZZ",
    })

    const filteredRows = rows.filter(
      (row) => normalizeId(pickFirst(row, ["id_medecin"])) === filterMedecinId
    )
    ...
    const affiliations = sortedRows.map((row, index) => {
      ...
      const medecinId = pickFirst(row, ["id_medecin"])
      const equipeNom = pickFirst(row, ["nom_equipe"])
      const equipeNationaleNom = pickFirst(row, ["nom_equipe_nationale"])

      return {
        __key: `${id || medecinId}__${index}`,
        id,
        medecinId,
        medecinNom: pickFirst(row, ["nom_coach"]) || "-",
    ...
    console.error("[api/medecin-affiliations]", message)
