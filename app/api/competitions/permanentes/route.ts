...
import { createPermanentCompetition, listCompetitionCatalog } from "@/lib/competition-catalog";
...
export async function GET() { if (!(await getSessionUser())) return NextResponse.json({ error: "Non authentifié." }, { status: 401 }); try { const data = await listCompetitionCatalog(); return NextResponse.json({ permanents: data.permanents, editions: data.editions }); } catch (error) { return fail(error); } }
export async function POST(request: Request) { const user = await getSessionUser(); if (!user) return NextResponse.json({ error: { message: "Authentification requise." } }, { status: 401 }); if (user.role !== "federal") return NextResponse.json({ error: { message: "Droit fédéral requis." } }, { status: 403 }); try { return NextResponse.json({ created: await createPermanentCompetition(await request.json().catch(() => null)) }, { status: 201 }); } catch (error) { return fail(error); } }
