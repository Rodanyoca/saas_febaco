...
import { createCompetitionEvent, listCompetitionEditionsCatalog } from "@/lib/competition-catalog";
...
export async function GET() { if (!(await getSessionUser())) return NextResponse.json({ error: "Non authentifié." }, { status: 401 }); try { return NextResponse.json(await listCompetitionEditionsCatalog()); } catch (error) { return fail(error); } }
